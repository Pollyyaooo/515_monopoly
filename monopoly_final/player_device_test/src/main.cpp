// =============================================================
// main.cpp  (PlatformIO — converted from player_device_test.ino)
...
void scanNFC() {
  // Only actively poll when on the NFC scan page
  if (currentPage != PAGE_NFC_SCAN) return;
  ...
  if (nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLen, 50)) {
    ...
    if (sq >= 0) {
      // Known card — update position
      game.players[game.currentTurnIdx].position = sq;
      Serial.printf("[NFC] Mapped -> square %d (%s)\n", sq, squares[sq].name);
    } else {
      // Unknown card — any tag counts as a valid scan; keep current position
      Serial.println("[NFC] Unknown tag — scan accepted, position unchanged");
    }

    // Any tag detected = scan success → go to ACTION
...
// Forward declaration — processButtons() calls handleButtonEvent()
...
void handleButtonEvent(uint8_t pin, ButtonEvent evt) {
  ...
  // Long-press OK on any page → advance to next turn, then prompt NFC scan
  ...
  switch (currentPage) {
    ...
    case PAGE_HOME:
      if (pin == BTN_OK    && evt == EVT_SHORT) { currentPage = PAGE_NFC_SCAN;   needRedraw = true; }
    ...
    case PAGE_ACTION:
      ...
      if (pin == BTN_OK   && evt == EVT_SHORT) {
        Player& cur = game.players[game.currentTurnIdx];
        Square& sq  = squares[cur.position];
        ...
        switch (sq.type) {
          ...
          case SQ_RAILROAD:
            if (sq.owner == -1) {
              // Buy the property
              cur.balance -= sq.price;
              sq.owner     = game.currentTurnIdx;
              Serial.printf("[PAY] %s bought %s for $%d  (bal=$%d)\n",
                            cur.name, sq.name, sq.price, cur.balance);
            } else if (sq.owner != game.currentTurnIdx) {
              // Pay rent to owner
              cur.balance                    -= sq.rentBase;
              game.players[sq.owner].balance += sq.rentBase;
              Serial.printf("[PAY] %s paid rent $%d to %s  (bal=$%d)\n",
                            cur.name, sq.rentBase,
                            game.players[sq.owner].name, cur.balance);
            }
            break;

          case SQ_TAX:
            cur.balance -= sq.rentBase;
            Serial.printf("[PAY] %s paid tax $%d  (bal=$%d)\n",
                          cur.name, sq.rentBase, cur.balance);
            break;

          case SQ_SPECIAL:
            if (cur.position == 0) {  // Go
              cur.balance += 200;
              Serial.printf("[PAY] %s collected $200 on Go  (bal=$%d)\n",
                            cur.name, cur.balance);
            } else if (cur.position == 5) {  // Jail — pay $50 bail
              cur.balance -= 50;
              Serial.printf("[PAY] %s paid $50 jail bail  (bal=$%d)\n",
                            cur.name, cur.balance);
            }
            break;
...
void drawHome() {
  ...
  display.print("P");
  display.print(game.currentTurnIdx + 1);
  display.print(": ");
...
void drawAction() {
  ...
  // Show current balance so player sees effect after payment
  display.setTextColor(C_GREEN);
